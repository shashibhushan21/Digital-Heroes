...
test.describe("Admin charity upload and public gallery visibility", () => {
  ...
  test("uploaded charity media appears on public charity profile", async ({ page }) => {
    test.skip(!setupReady, setupReason || "Admin auth setup not ready.");

    const marker = `e2e-gallery-${Date.now()}`;
    const altText = `E2E Gallery ${marker}`;
    ...
    const mediaForm = charityPanel.locator("form").first();

    await mediaForm.getByPlaceholder("Media URL").fill(imageDataUrl);
    await mediaForm.getByPlaceholder("Alt text").fill(altText);
    await mediaForm.getByPlaceholder("Caption").fill(marker);

    await mediaForm.getByRole("button", { name: "Add Media" }).click();

    await expect(charityPanel.locator("p", { hasText: altText }).first()).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.getByRole("img", { name: altText })).toBeVisible({ timeout: 15000 });
  });
});
